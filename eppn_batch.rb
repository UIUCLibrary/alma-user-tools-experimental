...
def update_user( api, user )
  ...
  # double-check scripts w/ ruby, think could make these @ and refer to them
  ...
  request.content_type =  'application/xml' 
    ...
    if response.code.to_i >= 400 and response.code.to_i < 600
      log.info("Got an error response (#{response.code}) on api call for " + user.primary_id)
...
library_employee_list.all().each do | person |
  ...
  if response.code.to_i >= 400 and response.code.to_i < 600
    log.info("Got an error response (#{response.code}) on api call for " + person["netid"])
    log.debug( response.body )
